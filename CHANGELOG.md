...
- The headings-inside setting is edited on the container element's own row in the heading-structure module: the select shows the container field's stored setting (with the effective level, e.g. "Automatic — next level: H3"), and changing it re-levels all of the container's children (and their linked descendants) with one change. A container that renders no heading of its own (empty header, `renderTag="false"`) still appears as a plain-text "Container without its own heading" row so the field stays reachable, with a "Contains no headings yet." note while nothing derives from it. Descendant- and sibling-derived headings are read-only and are marked by a trailing link icon (a lock when the source is not in the list) on their dashed level chip rather than by wordy chip text, so the heading title stays the row's most prominent element; the full explanation — which kind of element the level is inherited from, that it cannot be changed there, and where the jump leads — is carried as screen-reader text. Their jump button focuses the source heading's applicable selector ("Headings inside" for descendants, the heading-level selector for siblings). If permissions leave no source control available, the labelled source list item receives focus instead, so screen-reader users get the same destination as the visual highlight. Skipped heading levels caused by such a hidden container are reported once on the container's row — the row already shows the unrendered level and the select that fixes the gap — instead of as missing-level placeholders under every affected child.
...
- The heading-structure module marks findings directly on the affected row — a shared inline severity notice plus a colored accent bar and dot on the row itself — instead of separate full-width notice rows beneath (e.g. every duplicate `<h1>` row carries its "Multiple <h1> headings found" warning itself). Missing `<h1>`, missing-level placeholders, and errors attributed to hidden containers use that same notice and row structure; only their necessary context differs (page scope, level-specific text and indentation, or the container controls). Page-level heading findings without an affected element appear as their own rows in the same list. Missing-level placeholder rows remain the signal for a skipped level without an editable container row; the skipping heading's select announces the placeholder's message. The findings overview sits inside its own tab, above that structure, and shows each grouped issue's occurrence count in a distinct labelled badge — the heading and the landmark tab each list only their own findings, because activating one jumps to the affected row in the structure below it. Mobile-first rows group the level and title, give the headings-inside selector its own labelled field group on narrow screens and fold it flat into the row on wide ones, place status and actions consistently, and reduce deep indentation on narrow screens before progressively restoring it at wider widths. Rows that do not wrap all share one height, whichever controls they carry — a heading-level select, a read-only inherited chip, or the headings-inside setting. Every visual cue (viewport badges, save spinner, severity colors) carries visible or screen-reader text. The heading structure renders as a single flat list — each row announces its own heading level and is visually indented by it (containers indent one step above their children) — instead of nested lists whose depth could contradict the announced levels around skips and containers. The list is one framed surface rather than a stack of per-row boxes, and the levels are joined by a dotted tree line — a vertical rail down each level, a horizontal elbow into every row beneath it. Narrow layouts drop the lines and keep the indentation alone, where the indent step is too small to draw them apart. Editable options and read-only badges use the same labels: "H1 — page heading", "H2" through "H6", and "Paragraph — not a heading".
...
- The heading and landmark structure now reports its result as a status row in the same style as the alternative-text and scan rows of the accessibility info box — a message stating that structure issues were found (or confirming there are none), colored by the worst severity present. That row replaces the widget's own section heading and stands above the heading/landmark tabs in both places; it names the worst severity present for screen readers as well as by color, and where only one of the two structure checks is available the single view is a labelled region so it stays identifiable without the tabs. In the page module the structure itself is folded away behind that row, so the info box no longer pushes the content elements down the page; unfolding it is remembered in the browser and applies to the pages visited next. The Accessibility module's Overview keeps showing the full structure.
- The scan view's single panel — page-only mode, where there is no crawl tab to switch to — is now a labelled region as well, so screen-reader users can identify it without a tablist to name it.
- Findings hidden in an inactive tab — the scan view's other mode and the heading/landmark structure trees — are now reachable with the browser's find-in-page search: inactive panels are hidden with `hidden="until-found"`, and a match reveals the panel and activates its tab, keeping the tablist state in sync. Browsers without `until-found` support keep the previous fully-hidden behavior.
- The backend module scripts now assume an evergreen-browser baseline from mid-2024 (roughly Chrome/Edge 117+, Firefox 124+, Safari 17.4+): the structure-analysis and scan views build on platform APIs of that generation (`AbortSignal.any`/`.timeout`, `Map.groupBy`). Newer conveniences beyond the baseline degrade gracefully — find-in-page tab reveal and the compact scan row's self-hiding fall back cleanly — but the scan and structure views themselves no longer run on older engines.
- The alternative-text input in the Missing Alternative Text cards grows with its content in supporting browsers (`field-sizing`); manual resizing stays available everywhere. Long titles in scan results and notices now wrap balanced instead of leaving a lone word on the last line.
- Every row of the accessibility info box now states its count the same way: the number moved out of the message text into the issue-count badge already used elsewhere in the module, so the missing-alternative-text, external-scan and page-structure rows read alike and line up at the same width. Screen-reader announcements are unaffected — the external-scan status message still speaks its issue total, so a scan settling in the background is announced with the number and two scans differing only in count are announced separately.
...
- Editors with a reduced-motion preference keep the jump highlight that marks the row a findings-overview jump landed on: the highlight is a pure color fade without motion, but the global reduced-motion rule finished it instantly, removing the "where did I land" cue for exactly the users who rely on it. Reduced motion now disables actual motion effects only.
- The accessibility module and the page-module info box no longer flash unstyled notice text (bare message text with an unstyled link) while the module's scripts are still loading: the affected status notices reserve their space and appear fully styled, and should the scripts fail to load, the server-rendered content becomes visible after a short timeout instead of staying lost.
- The compact scan status row no longer removes a `hidden` attribute set by the embedding markup; when it has nothing to show it now hides itself without touching integrator-set attributes.
- Expanded violation cards in the scan results keep their expanded state and keyboard focus on the right card when a refreshed result lists the violations in a different order; previously the expanded state stuck to the list position instead of the violation.
...
- The two "View details" links in the accessibility info box are now distinguishable for screen-reader users: each carries the name of the section it opens, so a list of the page's links no longer shows two identically named entries.
